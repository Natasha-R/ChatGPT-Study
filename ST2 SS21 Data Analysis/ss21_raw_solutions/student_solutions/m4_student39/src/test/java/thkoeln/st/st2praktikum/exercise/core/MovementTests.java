...
public abstract class MovementTests extends GenericTests {
    ...
    protected void createWorld(MiningMachineService miningMachineService, FieldService fieldService, TransportTechnologyService transportTechnologyService) {
        ...
        connection1 = transportTechnologyService.addConnection(transportTechnology1, field1, Vector2D.fromString("(1,1)"), field2, Vector2D.fromString("(0,1)"));
        connection2 = transportTechnologyService.addConnection(transportTechnology1, field2, Vector2D.fromString("(1,0)"), field3, Vector2D.fromString("(2,2)"));
        connection3 = transportTechnologyService.addConnection(transportTechnology2, field3, Vector2D.fromString("(2,2)"), field2, Vector2D.fromString("(1,0)"));
    }
    ...
    protected void assertPosition(UUID miningMachineId, UUID expectedFieldId, int expectedX, int expectedY) throws Exception {
        ...
        // Assert Pos
        Method getVector2DMethod = miningMachine.getClass().getMethod("getVector2D");
        assertEquals(new Vector2D(expectedX, expectedY),
                getVector2DMethod.invoke(miningMachine));
    }
