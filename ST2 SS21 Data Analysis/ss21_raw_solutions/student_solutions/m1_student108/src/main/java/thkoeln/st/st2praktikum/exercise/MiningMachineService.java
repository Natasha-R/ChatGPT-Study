package thkoeln.st.st2praktikum.exercise;

import thkoeln.st.st2praktikum.exercise.FieldComponent.Connection;
import thkoeln.st.st2praktikum.exercise.FieldComponent.IComponent;
import thkoeln.st.st2praktikum.exercise.FieldComponent.MiningMachine;
import thkoeln.st.st2praktikum.exercise.FieldComponent.Wall;

import java.util.UUID;

public class MiningMachineService {
    UUIDManager uuidManager;
    CommandParser commandParser;
    ConnectionManager connectionManager;

    public MiningMachineService() {
        uuidManager = new UUIDManager();
        commandParser = new CommandParser();
        connectionManager = new ConnectionManager();
    }

    /**
     * This method creates a new field.
     * @param height the height of the field
     * @param width the width of the field
     * @return the UUID of the created field
     */
    public UUID addField(Integer height, Integer width) {
        Field field = new Field(width, height);
        uuidManager.rememberObject(field);
        return field.getUUID();
    }

    /**
     * This method adds a wall to a given field.
     * @param fieldId the ID of the field the wall shall be placed on
     * @param wallString the end points of the wall, encoded as a String, eg. "(2,3)-(10,3)" for a wall that spans from (2,3) to (10,3)
     */
    public void addWall(UUID fieldId, String wallString) {
        new Wall((Field) uuidManager.getObjectFromUUID(fieldId), wallString);
    }

    /**
     * This method adds a traversable connection between two fields. Connections only work in one direction.
     * @param sourceFieldId the ID of the field where the entry point of the connection is located
     * @param sourceCoordinate the coordinates of the entry point
     * @param destinationFieldId the ID of the field where the exit point of the connection is located
     * @param destinationCoordinate the coordinates of the exit point
     * @return the UUID of the created connection
     */
    public UUID addConnection(UUID sourceFieldId, String sourceCoordinate, UUID destinationFieldId, String destinationCoordinate) {
        int[] sourcePosition = commandParser.transformToIntArray(sourceCoordinate);
        int[] destinationPosition = commandParser.transformToIntArray(destinationCoordinate);
        return connectionManager.createConnection((Field) uuidManager.getObjectFromUUID(sourceFieldId), sourcePosition, (Field) uuidManager.getObjectFromUUID(destinationFieldId), destinationPosition, uuidManager).getUUID();
    }

    /**
     * This method adds a new mining machine
     * @param name the name of the mining machine
     * @return the UUID of the created mining machine
     */
    public UUID addMiningMachine(String name) {
        MiningMachine machine = new MiningMachine(name);
        uuidManager.rememberObject(machine);
        return machine.getUUID();
    }

    /**
     * This method lets the mining machine execute a command.
     * @param miningMachineId the ID of the mining machine
     * @param commandString the given command, encoded as a String:
     * "[direction, steps]" for movement, e.g. "[we,2]"
     * "[tr,field-id]" for transport, e.g. "[tr,4de4d85c-612c-4fa0-8e0f-7212f0711b40]" - only works on cells with a connection to another field
     * "[en,<field-id>]" for setting the initial field where a mining machine is placed, e.g. "[en,4de4d85c-612c-4fa0-8e0f-7212f0711b40]". The mining machine will always spawn at (0,0) of the given field.
     *
     * @return true if the command was executed successfully. Else false.
     *      (Movement commands are always successful, even if the mining machine hits a wall or
     *      another mining machine, and can move only a part of the steps, or even none at all.
     *      tr and en commands are only successful if the action can be performed.)
     */
    public Boolean executeCommand(UUID miningMachineId, String commandString) {
        String[] data = commandParser.transformToStringArray(commandString);
        MiningMachine miningMachine = (MiningMachine) uuidManager.getObjectFromUUID(miningMachineId);
        boolean result = false;
        switch (data[0]){
            case "no":
            case "ea":
            case "so":
            case "we":
                result = miningMachine.move(data[0], Integer.parseInt(data[1]));
                break;
            case "tr":
                Connection connectionToUse = (Connection) connectionManager.getConnection(miningMachine.getField().getUUID().toString(), data[1]);
                result = connectionManager.useConnection(connectionToUse.getUUID());
                break;
            case "en":
                result = setMiningMachineOnField(miningMachineId, data[1]);
                break;
            default:

        }
        return result;
    }

    private boolean setMiningMachineOnField(UUID miningMachineId, String fieldID) {
        IComponent findedField = uuidManager.getObjectFromUUIDAsString(fieldID);
        if (findedField == null) { return false; }
        Field field = (Field) findedField;
        if (field.addMachine((MiningMachine) uuidManager.getObjectFromUUID(miningMachineId))) {
            MiningMachine machine = (MiningMachine) uuidManager.getObjectFromUUID(miningMachineId);
            machine.setField(field);
            machine.setPosition(0, 0);
        } else { return false; }
        return true;
    }

    /**
     * This method returns the field-ID a mining machine is standing on
     * @param miningMachineId the ID of the mining machine
     * @return the UUID of the field the mining machine is located on
     */
    public UUID getMiningMachineFieldId(UUID miningMachineId){
        MiningMachine machine = (MiningMachine) uuidManager.getObjectFromUUID(miningMachineId);
        return machine.getField().getUUID();
    }

    /**
     * This method returns the coordinates a mining machine is standing on
     * @param miningMachineId the ID of the mining machine
     * @return the coordiantes of the mining machine encoded as a String, e.g. "(2,4)"
     */
    public String getCoordinates(UUID miningMachineId){
        MiningMachine machine = (MiningMachine) uuidManager.getObjectFromUUID(miningMachineId);
        return machine.getPositionAsString();
    }
}
