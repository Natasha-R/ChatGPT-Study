...
import thkoeln.st.st2praktikum.exercise.FieldBlockable;
...
public class TidyUpRobot extends AbstractRobot implements Transportable
{
    ...
    public boolean moveInDirection(TaskType key, Integer steps, Connectable connection, Roomable room)
    {
        switch (key)
        {
            ...
            case TRANSPORT:
                return transportToRoom(room,connection);
            case NORTH:
    ...
    private boolean transportToRoom(Roomable destinationRoom, Connectable usedConnection)
    {
    ...
    private boolean isTransportable(Connectable usedConnection)
    {
        ...
        return coordinate.getCoordinateX() == connectionPoint.getCoordinateX() &&
                coordinate.getCoordinateY() == connectionPoint.getCoordinateY();
