...
public class WallManager {
    ...
    public Field registerWall(Field field, OrderType orderType, int from, int to, int base) {
        for (int i = from; i < to; i++) {
            switch (orderType) {
                case NORTH:
                case SOUTH:
                    SingleWall wall = new SingleWall(base, i);
                    field.getVerticalWalls().add(wall);
                    System.out.println("(" + wall.getX() + "," + wall.getY() + ")");
                    break;
                case EAST:
                case WEST:
                    wall = new SingleWall(i, base);
                    field.getHorizontalWalls().add(wall);
                    System.out.println("(" + wall.getX() + "," + wall.getY() + ")");
                    break;
            }
        }
    ...
    public int calculateNorth(Field field, int value, int x, int y) {
        ...
        int maxHeight = field.getEnd().getY();
    ...
    public int calculateEast(Field field, int value, int x, int y) {
        ...
        int maxWidth = field.getEnd().getX();
