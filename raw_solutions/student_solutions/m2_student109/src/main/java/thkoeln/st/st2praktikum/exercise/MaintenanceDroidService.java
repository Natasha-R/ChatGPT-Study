...
public class MaintenanceDroidService {
    ...
    public UUID addConnection(UUID sourceSpaceShipDeckId, Vector2D sourceVector2D, UUID destinationSpaceShipDeckId, Vector2D destinationVector2D) {
        ...
        for (SpaceShipDeck spaceShipDeck : spaceShipDecks) {
            if (spaceShipDeck.getSpaceShipDeskId().equals(sourceSpaceShipDeckId)) {
                ...
                spaceShipDeck.addConnection(new Connection(sourceSpaceShipDeckId, sourceVector2D, destinationSpaceShipDeckId, destinationVector2D, connectionId));
            }
        }
        return connectionId;
    }
                ...
                for (MaintenanceDroid maintenanceDroid : maintenanceDroids) {
                    if (maintenanceDroid.getMaintenanceDroidsId().equals(maintenanceDroidId)) {
                        if (spaceShipDeck.checkPositionForDroid(maintenanceDroids, 0, 0)) {
                            ...
                            maintenanceDroid.setPosition(new Vector2D(0, 0));
                            return true;
                        }
                    }
                }
            ...
            if (maintenanceDroid.getMaintenanceDroidsId().equals(maintenanceDroidId)) {
                for (SpaceShipDeck spaceShipDeck : spaceShipDecks) {
                    if (maintenanceDroid.getSpaceShipDeskId().equals(spaceShipDeck.getSpaceShipDeskId())) {
                        ...
                        if (maintenanceDroid.getSpaceShipDeskId().equals(tmp.getMaintenanceDroidsId())) {
    ...
    public Vector2D getVector2D(UUID maintenanceDroidId) {
        for (int i = 0; i < maintenanceDroids.size(); i++) {
            if (maintenanceDroids.get(i).getMaintenanceDroidsId().equals(maintenanceDroidId)) {
                return maintenanceDroids.get(i).getPosition();
            }
        }
        return null;
    }
