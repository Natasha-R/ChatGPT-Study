...
public abstract class MovementTests {
    ...
    protected void createWorld(MiningMachineService service) {
        ...
        connection1 = service.addConnection(field1, new Vector2D("(1,1)"), field2, new Vector2D("(0,1)"));
        connection2 = service.addConnection(field2, new Vector2D("(1,0)"), field3, new Vector2D("(2,2)"));
        connection3 = service.addConnection(field3, new Vector2D("(2,2)"), field2, new Vector2D("(1,0)"));
    }
    ...
    protected void assertPosition(MiningMachineService service, UUID miningMachineId, UUID expectedFieldId, int expectedX, int expectedY) {
        ...
        assertEquals(new Vector2D(expectedX, expectedY),
                service.getVector2D(miningMachineId));
    }
