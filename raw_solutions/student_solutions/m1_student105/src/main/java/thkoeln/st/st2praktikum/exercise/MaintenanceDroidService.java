...
import thkoeln.st.st2praktikum.exercise.entities.commands.Command;
...
public class MaintenanceDroidService {
    ...
    public void addBarrier(UUID spaceShipDeckId, String barrierString) {
        ship.findDeckById(spaceShipDeckId).addBarrier(barrierString);
    }
    ...
    public Boolean executeCommand(UUID maintenanceDroidId, String commandString) {
        Command c = Command.fromString(commandString, maintenanceDroidId, ship);
        return c.executeOn(ship);
    }
    ...
    public String getCoordinates(UUID maintenanceDroidId){
        return ship.findDroidById(maintenanceDroidId).getCurrentPosition().toString();
    }
}
