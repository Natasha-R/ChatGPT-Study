...
public class Device extends UUIDElement implements Commandable, Enterable, Movable, Traversable, Observable<EnvironmentPosition> {
    ...
    public void enter(UUID spaceId, Position entryPosition) throws FailedExecutionException {
        ...
        if (!sensor.isOnSpace(entryEnvironmentPosition)) {
            throw new EntryPositionOutOfSpaceException(entryEnvironmentPosition);
        } else if (!sensor.isPositionFree(entryEnvironmentPosition)) {
    ...
    @Override
    public void traverse(UUID destinationSpaceId) throws NoConnectedTransitionException, BlockedMoveException, EntryPositionOutOfSpaceException {
        ...
        } else if (!sensor.isOnSpace(connectedPositionOptional.get())) {
            throw new EntryPositionOutOfSpaceException(connectedPositionOptional.get());
