...
public class MiningMachineService {
    ...
    private List<Movable> miningMachines = new ArrayList<>();
    /**
    ...
    public void addWall(UUID fieldId, String wallString) {
        ...
        for (Field value : fields) {
            if (value.getFieldId().equals(fieldId)) targetField = value;
        }
        if(targetField != null) {
            targetField.getWalls().add(new Wall(wallString));
        }
    }
    ...
    public Boolean executeCommand(UUID miningMachineId, String commandString) {
        Movable miningMachine = null;
        for (Movable machine : miningMachines) {
            if (machine.getId().equals(miningMachineId)) miningMachine = machine;
        }
        if(miningMachine == null)throw new NullPointerException();
        return miningMachine.executeCommand(commandString,fields,miningMachines);
    }
    ...
    public String getCoordinates(UUID miningMachineId){
        for (Movable machine : miningMachines) {
            if (machine.getId().equals(miningMachineId)) {
                return "(" + machine.getX() + "," + machine.getY() + ")";
            }
        }
        throw new NullPointerException();
    }
