...
public class DroidManager {
    ...
    private final TransportSystemManager transportSystemManager;
    private final DroidMovementService movementService = new DroidMovementService();


    public DroidManager(DeckManager deckManager, TransportSystemManager transportSystemManager) {
        this(new ArrayList<>(), deckManager, transportSystemManager);
    }

    public DroidManager(ArrayList<MaintenanceDroid> droids, DeckManager deckManager, TransportSystemManager transportSystemManager) {
        this.maintenanceDroids = droids;
        this.deckManager = deckManager;
        this.transportSystemManager = transportSystemManager;
    }
    ...
    public Boolean useConnection(MaintenanceDroid droid, UUID newDeckId) {
        ArrayList<Connection> deckConnections = this.transportSystemManager.getConnectionsBySourceDeckID(droid.getSpaceShipDeckId());
    ...
    private Boolean traverseConnection(MaintenanceDroid droid, Connection connection) {
        SpaceShipDeck newDeck = this.deckManager.getSpaceShipDeckByUUID(connection.getDestinationSpaceShipDeckId());
    ...
    public ArrayList<MaintenanceDroid> getMaintenanceDroidsByDeckId(UUID deckId) {
        ...
        return this.maintenanceDroids.stream().filter(b -> b.getSpaceShipDeckId() != null)
                .filter(b -> b.getSpaceShipDeckId().equals(deckId))
