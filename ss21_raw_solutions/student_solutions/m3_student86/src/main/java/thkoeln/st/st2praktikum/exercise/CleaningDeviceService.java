...
public class CleaningDeviceService {
    private final Map<UUID, Space> spaceMap = new HashMap<>();
    private final Map<UUID, CleaningDevice> cleaningDeviceMap = new HashMap<>();
    private final Map<UUID, TransportCategory> transportCategoryMap = new HashMap<>();
    ...
    public CleaningDevice getCleaningDevice (UUID cleaningDeviceId) {
        CleaningDevice device = cleaningDeviceMap.get(cleaningDeviceId);
        if (device == null) throw new RuntimeException(
                "CleaningDevice " + cleaningDeviceId.toString() + " existiert nicht");
        return device;
    }

    public TransportCategory getTransportCategory (UUID transportCategoryId) {
        TransportCategory category = transportCategoryMap.get(transportCategoryId);
        if (category == null) throw new RuntimeException(
                "TransportCategory " + transportCategoryId.toString() + " existiert nicht");
        return category;
    }
    ...
    public UUID addTransportCategory(String category) {
        ...
        transportCategoryMap.put(categoryId, transportCategory);
        return categoryId;
    }
    ...
    public UUID addConnection(UUID transportCategoryId, UUID sourceSpaceId, Vector2D sourceVector2D, UUID destinationSpaceId, Vector2D destinationVector2D) {
        TransportCategory category = getTransportCategory(transportCategoryId);
    ...
    public UUID addCleaningDevice(String name) {
        ...
        cleaningDeviceMap.put(connectionId, cleaningDevice);
        return connectionId;
    }
    ...
    public Boolean executeCommand(UUID cleaningDeviceId, Task task) {
        ...
        switch (task.getTaskType()) {
            case TRANSPORT: {
                ...
                List<TransportCategory> transportCategoryList =
                        new ArrayList<TransportCategory>(transportCategoryMap.values());

                try { cleaningDevice.transportToSpace(transportCategoryList, space);
    ...
    public void saveAll() {
        for (Space space : spaceMap.values()) spaceRepo.save(space);
        for (CleaningDevice device : cleaningDeviceMap.values()) cleaningDeviceRepo.save(device);
        for (TransportCategory category : transportCategoryMap.values()) transportCategoryRepo.save(category);
    }

    public void loadAll() {
        spaceMap.clear();
        cleaningDeviceMap.clear();
        transportCategoryMap.clear();

        for (Space space : spaceRepo.findAll())
            spaceMap.put(space.getId(), space);
        for (CleaningDevice device : cleaningDeviceRepo.findAll())
            cleaningDeviceMap.put(device.getId(), device);
        for (TransportCategory category : transportCategoryRepo.findAll())
            transportCategoryMap.put(category.getId(), category);
    }

    private void deleteAll() {
        transportCategoryRepo.deleteAll();
        cleaningDeviceRepo.deleteAll();
        spaceRepo.deleteAll();
    }
}
