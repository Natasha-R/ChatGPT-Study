...
public class MiningMachine implements Transportable,Initializable, Moveable{
   ...
   // }
    ...
    @Override
    public boolean transport(HashMap<UUID,Connection> connections, UUID fieldUUID) {
    ...
    @Override
    public boolean initialize(HashMap<UUID,MiningMachine> miningMachines, UUID fieldID) {
        ...
        for(int i=0;i<numberOfSteps;i++)
        {
            ...
            if(!moveBlocked)
            {
                ...
                for(Map.Entry miningMachine : miningMachines.entrySet()){
                    if(miningMachines.get(miningMachine.getKey()).getPos().equals(moveTo))
                    {
