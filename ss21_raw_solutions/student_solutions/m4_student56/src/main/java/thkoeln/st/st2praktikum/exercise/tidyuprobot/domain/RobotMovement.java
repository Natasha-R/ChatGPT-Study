...
public class RobotMovement {
    ...
    public Boolean executeCommand(UUID tidyUpRobotID, Command command, TidyUpRobotRepository tidyUpRobotRepository, RoomRepository roomRepository, ConnectionRepository connectionRepository) {
        ...
        switch (command.getCommandType()) {
            ...
            case NORTH:
                return goNorth(tidyUpRobotID, command, tidyUpRobotRepository, roomRepository);

            case EAST:
                return goEast(tidyUpRobotID, command, tidyUpRobotRepository, roomRepository);

            case WEST:
                return goWest(tidyUpRobotID, command, tidyUpRobotRepository, roomRepository);

            case SOUTH:
                return goSouth(tidyUpRobotID, command, tidyUpRobotRepository, roomRepository);

            case ENTER:
                return enter(tidyUpRobotID, command, tidyUpRobotRepository,roomRepository);

            case TRANSPORT:
                return transport(tidyUpRobotID, command, connectionRepository, tidyUpRobotRepository);
    ...
    public Boolean goNorth(UUID tidyUpRobotID, Command command, TidyUpRobotRepository tidyUpRobotRepository, RoomRepository roomRepository) {
        ...
        tidyUpRobotRepository.findById(tidyUpRobotID).get().getCommands().add(command);
        System.out.println(tidyUpRobotRepository.findById(tidyUpRobotID).get().getCommands());
        ...
        Coordinate coordinate = tidyUpRobotRepository.findById(tidyUpRobotID).get().getCoordinate();
        return true;
    }
    ...
    public Boolean enter(UUID tidyUpRobotID, Command command, TidyUpRobotRepository tidyUpRobotRepository , RoomRepository roomRepository) {


        tidyUpRobotRepository.findById(tidyUpRobotID).get().getCommands().add(command);
        TidyUpRobot tidyUpRobot = tidyUpRobotRepository.findById(tidyUpRobotID).get();
        Room room = roomRepository.findById(command.getGridId()).get();
        ...
        TidyUpRobot tidyUpRobot1 = tidyUpRobots.get(0);
        TidyUpRobot tidyUpRobot2= tidyUpRobots.get(1);
        long a = tidyUpRobotRepository.count();
    ...
    public Boolean moveVertikalIntoBarrier(Coordinate coordinate, Room room) {
        ...
        for (int i = 0; i < room.getHorizontalBarriers().size(); i++) {
            if (coordinate.getY().equals(room.getHorizontalBarriers().get(i).getStart().getY()) && coordinate.getX() - room.getHorizontalBarriers().get(i).getStart().getX() + room.getHorizontalBarriers().get(i).getEnd().getX() - coordinate.getX() == room.getHorizontalBarriers().get(i).getEnd().getX() - room.getHorizontalBarriers().get(i).getStart().getX()) {
                if ( !coordinate.getX().equals(room.getHorizontalBarriers().get(i).getEnd().getX())) {
                    return false;
                }
            }
        }
