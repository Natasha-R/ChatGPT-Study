...
public class Movement implements Movable {
    ...
    private final Map<UUID, TransportTechnology> transportTechnologies;
    private final Map<UUID, Connection> connections;

    public Movement(Map<UUID, MiningMachine> miningMachines, Map<UUID, Field> fields, Map<UUID, TransportTechnology> transportTechnologies, Map<UUID, Connection> connections) {
        this.miningMachines = miningMachines;
        this.fields = fields;
        this.transportTechnologies = transportTechnologies;
    ...
    public Boolean moveToAnotherField(UUID miningMachineId, UUID destinationFieldId) {
        ...
        for (Map.Entry<UUID, Connection> f : connections.entrySet()) {
            if (f.getValue().getSourceFieldId() == fields.get(sourceFieldId).getId()
                    ...
                    && f.getValue().getSourceCoordinate().getX().equals(machine.getCoordinate().getX())
                    && f.getValue().getSourceCoordinate().getY().equals(machine.getCoordinate().getY())) {
    ...
    public Boolean checkMachineMap(Coordinate position, UUID incomingFieldID) {
        ...
        for (Map.Entry<UUID, MiningMachine> f : miningMachines.entrySet()) {
            if (f.getValue().getCoordinate() != null) {
                occupied = f.getValue().getCurrentField().getId() == fields.get(incomingFieldID).getId()
                        && position.getX().equals(f.getValue().getCoordinate().getX())
                        && position.getY().equals(f.getValue().getCoordinate().getY());
            }
        }
        return occupied;
    }
