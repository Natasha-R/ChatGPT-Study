...
public class Universe implements UniverseInterface {
    ...
    private final HashMap<UUID, SpaceShipDeck> spaceShipDeckHashMap = new HashMap<>();
    private final HashMap<UUID, MaintenanceDroid> maintenanceDroidHashMap = new HashMap<>();
    ...
    public void addObstacle(UUID spaceShipDeckId, Obstacle obstacle) {
        ...
        ObstacleInterface resultObstacle = new MyObstacle(obstacle);
        if (resultObstacle.getStartX().equals(resultObstacle.getEndX())) {
            for (int i = resultObstacle.getStartY(); i < resultObstacle.getEndY(); i++) {
                spaceShipDeckHashMap.get(spaceShipDeckId).getAllObstacles().add(new MyObstacle(new Obstacle(new Coordinate(resultObstacle.getStartX(), i), new Coordinate(resultObstacle.getEndX(), i + 1))));
            }
        }
        if (resultObstacle.getStartY().equals(resultObstacle.getEndY())) {
            for (int i = resultObstacle.getStartX(); i < resultObstacle.getEndX(); i++) {
                spaceShipDeckHashMap.get(spaceShipDeckId).getAllObstacles().add(new MyObstacle(new Obstacle(new Coordinate(i, resultObstacle.getStartY()), new Coordinate(i + 1, resultObstacle.getEndY()))));
            }
        }
    }
    ...
    public UUID addConnection(UUID sourceSpaceShipDeckId, Coordinate sourceCoordinate, UUID destinationSpaceShipDeckId, Coordinate destinationCoordinate) {
        ...
        try {
            spaceShipDeckHashMap.get(sourceSpaceShipDeckId).findSquareAt(StringService.translateCoordinateToMyCoordinate(sourceCoordinate)).setConnection(connection);
...
//            System.out.println(e.toString());
    ...
    public Boolean executeCommand(UUID maintenanceDroidId, Order order) {
        CommandInterface command = StringService.translateOrderToCommand(order);
...
//            System.out.println(e.toString());
    ...
    public Coordinate getCoordinates(UUID maintenanceDroidId) {
        try {
            ...
            return StringService.translateMyCoordinateToCoordinate(tmpSquare.getCoordinates());
        } catch (MaintenanceDroidPositionIsNull e) {
//            System.out.println(e.toString());
            return null;
        }
